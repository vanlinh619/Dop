...
const STATE_ADD_TENANT = {CLOSED: 'CLOSED', ADDING: 'ADDING', WAITING: 'WAITING'}
const tenantsStore = useTenantStore()
const state = ref(STATE_ADD_TENANT.CLOSED)
...
const addTenant = async () => {
  const tenant = newTenantName.value?.trim()
  if (!tenant) return

  state.value = STATE_ADD_TENANT.WAITING
  try {
    await api.post('/api/v1/manage/tenant', {name: tenant})
    tenantsStore.tenants.push(tenant)
  } finally {
    newTenantName.value = ''
  ...
  <div class="flex items-center relative">
    <Listbox v-model="tenantSelected">
      ...
      <ListboxOptions
        ...
        <ListboxOption v-for="tenant in tenants" :key="tenant" :value="tenant" v-slot="{active}" as="template">
          <li :class="['px-4 py-2 cursor-pointer border-b border-slate-100', active ? 'bg-emerald-600 text-white font-medium' : '', tenantSelected === tenant ? 'bg-emerald-600 hover:bg-emerald-700 text-white font-medium' : '']">
            {{ tenant }}
          </li>
        </ListboxOption>
        ...
        <li v-if="state === STATE_ADD_TENANT.CLOSED" @click="state = STATE_ADD_TENANT.ADDING"
          ...
          <div class="flex justify-center">Thêm tenant</div>
        </li>
        ...
        <li v-if="state === STATE_ADD_TENANT.WAITING"
            class="px-4 py-2 text-slate-500 font-medium border-t border-slate-200">
          <div class="flex items-center justify-center">
            <svg class="animate-spin -ml-1 mr-2 h-3 w-3 text-slate-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                 viewBox="0 0 24 24">
