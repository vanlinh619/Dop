...
public interface UserPrimaryRepository extends JpaRepository<UserPrimary, UUID> {
    ...
    @Query("""
    select new org.dop.module.user.pojo.projection.UserAuthenticatedProjection(u.id, u.password, u.status)
    from UserPrimary u
    ...
    @Query("""
    select new org.dop.module.user.pojo.projection.Auth2UserAuthenticatedProjection(u.id, u.status)
    from UserPrimary u
